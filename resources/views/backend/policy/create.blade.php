        ...
        <div class="col-md-12">
            <div class="panel panel-success">
                ...
                <div class="panel-wrapper">
                    <form action="{{ route('backend.policy.store') }}" class="form-horizontal" method="post" rel-action="submit">
                        <div class="panel-body p-b-0">
                            <div class="form-group row">
                                <label class="col-md-2 control-label col-form-label">所属策略</label>
                                <div class="col-md-10">
                                    <select class="form-control" name="module" rel-action="select">
                                        @foreach($filter['modules'] as $key => $module)
                                            <option value="{{ $module->getKey() }}">{{ $module->title }}</option>
                                            @foreach($module->policies as $key => $policies)
                                                <option value="{{ $module->getKey() }},{{ $policies->getKey() }}">{{ $module->title }}>{{ $policies->title }}</option>
                                                @foreach($policies->policies as $key => $policy)
                                                    <option value="{{ $module->getKey() }},{{ $policies->getKey() }},{{ $policy->getKey() }}">{{ $module->title }}>{{ $policies->title }}>{{ $policy->title }}</option>
                                                @endforeach
                                            @endforeach
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    <div class="input-group">
                                        ...
                                        <input class="form-control" type="text" name="icon" value="icon-settings" placeholder="请输入图标" autocomplete="off">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group row">
