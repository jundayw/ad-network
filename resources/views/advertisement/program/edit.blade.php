        ...
        <div class="col-md-12">
            <div class="panel panel-danger">
                ...
                <div class="panel-wrapper">
                    <form action="{{ route('advertisement.program.update') }}" class="form-horizontal" method="post" rel-action="submit">
                                    ...
                                    <div class="radio-list">
                                        @foreach($filter['device'] as $key => $device)
                                            <label class="radio-inline">
                                                <input type="radio" name="device" value="{{ $key }}" @checked($data->device == $key)>
                                                {{ $device }}
                                            </label>
                                        @endforeach
                                    </div>
