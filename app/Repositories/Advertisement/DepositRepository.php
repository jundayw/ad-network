...
class DepositRepository extends Repository
{
    ...
    public function list(Request $request): array
    {
        $data = $this->deposit
            ...
            ->where([
                'deposit_id' => $request->user()->getAttribute('advertisement_id'),
                'deposit_type' => 'advertisement',
            ])
    ...
    public function store(Request $request): Deposit
    {
        ...
        $request->user()->advertisement->increment('frozen', $deposit->getRawOriginal('amount'));
    ...
    public function update(Request $request): Deposit
    {
        ...
        $advertisement = $this->advertisement->find($request->user()->getAttribute('advertisement_id'));

        if (is_null($advertisement)) {
            throw new RenderErrorResponseException('暂无记录');
        }

        if ($request->get('amount') > $advertisement->getAttribute('balance')) {
        ...
        $request->user()->advertisement->decrement('balance', $deposit->getRawOriginal('amount'));
        $request->user()->advertisement->increment('frozen', $deposit->getRawOriginal('amount'));
